...
export async function generateStructuredContent(topic, grade){
  ...
  const sys = `You are a helpful assistant that outputs ONLY JSON. Produce a JSON object with keys: summary, keyPoints, quiz. summary: a detailed explanation of 3-4 paragraphs suitable for a grade ${grade} student. keyPoints: an array of 3-5 short bullets. quiz: array of 8 questions, each question object must have 'question', 'options' (4 strings), and 'correctAnswer' (string). Respond with JSON only.`
...
export async function repairToJson(malformed, topic, grade){
  const sys = `You are a JSON fixer. Given some text, extract and return a valid JSON object that matches this schema: {summary: string, keyPoints: [string], quiz: [{question:string, options:[string], correctAnswer: string}]}. Ensure there are 8 quiz questions. Do NOT include any extra fields.`
